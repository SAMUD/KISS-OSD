...
bool getSerialData(uint8_t Mode,bool CopyBuffToSett)	//reading serial Data from FC - Mode0:Telemetrie | Mode1:PID Settings
{
	...
	SerialCom.write(Mode);	//request data from FC
	
	//aquire serial data and write it to normal variables
	while (recBytes < minBytes)
	{
		...
		if (recBytes == minBytes)
		{
			...
			if (kissProtocolCRC8(serialBuf, STARTCOUNT, minBytes-1) == serialBuf[recBytes - 1] &&  Mode == GET_TELEMETRY)
			{
				...
				//do a fast data check
				for (i = 0; i < 4; i++)
				{
					if (KissTelemetrie.ESCTemps[i] > 160 || KissTelemetrie.motorCurrent[i] > 10000 ||
						KissTelemetrie.motorKERPM[i] > 700 || KissTelemetrie.AuxChanVals[i]>2005)
						return false;
				}
				if (KissTelemetrie.LipoVoltage > 10000)
					return false;
...
bool setSerialData()
{
	...
	//Re-aquire settings from FC (try to do it 10 times)
	i = 0;
	...
	SerialCom.write(SET_SETTINGS);
	//delay(2);

	//Alternative Mode:
	for (i = 0; i < KissSettings.minBytesSettings; i++)
	{
		SerialCom.write(serialBuf[i]);
	}
